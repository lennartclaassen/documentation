...
In diesem Kapitel werden die Elemente erläutert, aus welchen das entwickelte \kps{} aufgebaut wurde. Neben den verwendeten technischen Komponenten werden der Aufbau und die Kalibrierung des \kps{s} beschrieben. Darüber hinaus werden die verwendeten Softwarebibliotheken und die entwickelte Programmstruktur dargestellt.
...
\nomenclature[l]{$j,k$}{Variablen}
\nomenclature[l]{${^{j}\vec{T}_{k}}$}{Homogene Transformation vom $\ind{(KS)}_j$ ins $\ind{(KS)}_k$}
%\nomenclature[l]{$\ks{j},\ks{k}$}{Variable Koordinatensysteme}
\nomenclature[l]{$\ks{0}$}{Globales Koordinatensystem}
\nomenclature[l]{$\ks{K}$}{KS der RGB-Kamera der Kinect}
\nomenclature[l]{$\ks{K_\ind{IR}}$}{KS der IR-Kamera der Kinect}
\nomenclature[l]{$\ks{P_\ind{IR}}$}{KS des IR-Projektors der Kinect}
\nomenclature[l]{$\ks{KPS}$}{Basis-KS des \kps{s}}
\nomenclature[l]{$\ks{P}$}{KS des Pico-Laser-Projektors}
\nomenclature[l]{$\ks{M}$}{KS der Lokalisationsumgebung}
\nomenclature[l]{$\ks{Env}$}{KS der Modellumgebung}
\nomenclature[l]{$\ks{Odom}$}{Odometrie-KS für die Bewegung des \kps{s} im Raum}
\nomenclature[l]{$\ks{MF}$}{KS des Markerfeldes}
\nomenclature[l]{$\ks{K_\ind{ext}}$}{KS der externen Kamera}
...
\nomenclature[l]{$\rmat{j}{k}$}{Rotationsmatrix vom $\ks{j}$ ins $\ks{k}$}
...
\nomenclature[l]{$s_x$, $s_y$}{Umrechnungsfaktoren zwischen metrischen und Pixel-Koordinaten}
...
\nomenclature[l]{$\vec{r}_i$}{i-ter Spaltenvektor der Rotationsmatrix}
...
Die kontinuierlich bestimmte Pose wird an das Modul \textit{Visualisierung} übertragen, welches daraufhin die Lage des \kps{s} innerhalb der Modellumgebung ermittelt. Durch Bestimmung der perspektivischen Abbildung ausgewählter Modelldaten wird anschließend die Projektorsicht simuliert und dem Benutzer über das \kps{} durch Projektion visualisiert.\\ 
