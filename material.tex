...
Der Kinect Sensor verfügt über zwei Kameras, mit denen Videoaufnahmen im Farb- (RGB) sowie im Infrarotbereich (IR) möglich sind. Die Technik zur Ermittlung der Tiefeninformationen stammt von der Firma PrimeSense (PrimeSense LTD, Tel Aviv, Israel) und ist unter anderem durch das Patent \cite{Freedman2008} geschützt.\\
Die IR-Kamera wird dabei in Verbindung mit einem IR-Projektor verwendet um Tiefenbilder zu generieren. Der IR-Projektor projiziert ein irreguläres, bekanntes IR Muster, welches von der IR-Kamera erkannt wird. Über die Verzerrungen des Musters sind anschließend softwareseitig Rückschlüsse auf die Tiefenwerte der aufgenommenen Szene möglich. Dazu wird der Projektor als inverse Kamera betrachtet, wodurch der Aufbau des Kinect-Sensors dem einer Stereokamera entspricht. Da die Bildachsen der IR-Kamera und des IR-Projektors parallel orientiert sind, wird ein Punkt im projizierten Muster auf der in \red[\abb{fig.epipolar}] dargestellten horizontalen Epipolarlinie \red[$e$] im Kamerabild abgebildet. 
...
Da Laser verwendet werden ist die Schärfe des Bildes unabhängig vom Abstand zur Projektionsfläche. Der Einsatz von Linsen zur Fokussierung der Strahlen ist daher nicht erforderlich und auch eine manuelle Anpassung der Fokuseinstellungen muss nicht vorgenommen werden. \red[TODO: Funktionsprinzip] Genauere Spezifikationen des ShowWX finden sich in Anhang \ref{app:projector}.
