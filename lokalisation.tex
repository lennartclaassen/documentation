...
\begin{equation}
\label{eq.likelihood}
p(\vec{d}_{\ind{m}}|\vec{x}_{\ind{p}},m) = \prod_{i=1}^{n}p(d_{\ind{m,i}}|\vec{x}_{\ind{p}},m) \; \text{.}
\end{equation}
...
\end{enumerate}
Die visuelle Odometrie liefert durch das beschrieben Verfahren ein Ergebnis für die Veränderung der aktuellen Pose bezüglich aller sechs Freiheitsgrade. \abb{fig.fovis} (b) zeigt die aus zwei aufeinander folgenden Bildern ermittelten Merkmale, anhand welcher die Veränderung der Pose bestimmt werden kann.\\
