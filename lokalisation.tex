...
\begin{figure}[!ht]
	...
	\subfigure[\textit{Octomap}]{
		\includegraphics[scale=0.2]{octomap_rviz}
	}
	\hspace{5mm}
	\subfigure[Punktwolke]{
		\includegraphics[scale=0.2]{pointcloud_meshlab}	
	}
...
\begin{equation}
\mathcal{N}(d_{\ind{m}}^i;d_{\ind{k}},\sigma_{\ind{S}}^2) = \frac{1}{\sqrt{2 \cdot \pi \cdot \sigma_{\ind{S}}^2}}  \cdot e^{-\dfrac{1}{2} \cdot \dfrac{{({\Delta d}_i)}^2}{\sigma_{\ind{S}}^2}}
\end{equation}
...
& = \sum_{i=1}^{n}\log \left( \frac{1}{\sqrt{2 \cdot \pi \cdot \sigma_{\ind{S}}^2}} \cdot e^{-\dfrac{1}{2} \cdot \dfrac{{({\Delta d}_i)}^2}{\sigma_{\ind{S}}^2}} \right)\\[1em]
...
Erfolgt die Lokalisation wie in dieser Arbeit innerhalb einer statischen Umgebung kann die Berechnung der Distanz-Karte bereits vor der Lokalisation erfolgen. In jedem Schritt der Lokalisation kann dadurch eine deutliche Verringerung des Rechenaufwands erzielt werden. Anzumerken bleibt, dass die Distanz-Karte während der Programmlaufdauer im Speicher behalten wird und bezüglich des Speicherbedarfs die Ausgangskarte deutlich übersteigen kann.\\
